...
void PolynomialItem::updateGeometry()
{
  QList<QQuickItem*> children(childItems());
  std::for_each(children.begin(), children.end(),
                [] (QQuickItem* a_Item)
                {
                  PolynomialItem* item(qobject_cast<PolynomialItem*>(a_Item));
                  if(item)
                  {
                    item->updateGeometry();
                  }
...
QColor PolynomialItem::borderColor() const // TODO: this is the border color; the brush is responsible for the background color
{
  return m_Color;
}
